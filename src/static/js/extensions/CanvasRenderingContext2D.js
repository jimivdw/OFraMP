$ext.extend($ext, {
  context: {
    ...
    dashedLine: function(ctx, x1, y1, x2, y2, n) {
      ...
      var ddy = dy * l / dist;
      for( var i = 0; i < n; i++) {
        this.line(ctx, x1 + 2 * i * ddx, y1 + 2 * i * ddy, x1 + (2 * i + 1)
            * ddx, y1 + (2 * i + 1) * ddy);
      }
    },

    /*
     * Get the lines of a given text, provided that is has to fit in a box of
     * size w * h. It will split inside words if hard_wrap is set to true.
     * 
    ...
    /*
     * Show a given text at the position (x, y), while making sure it fits in
     * the box of size w * h.
     */
