...
(function() {
  var MAX_LINES = 1000;
  var CONSOLE_CSS = "#SHIMCONSOLE { visibility: hidden; position: fixed; z-index: 9999; left: 0; right: 0; bottom: 0; height: 200px; border-top: solid 1px #808080; overflow: auto; word-wrap: break-word; padding: 5px; background-color: #eeeeee; color: #000000; font-family: monospace; font-size: 10pt; font-weight: normal; font-style: normal; }"
      ...
      + "#SHIMCONTROLS { visibility: hidden; position: fixed; z-index: 9999; left: 0; right: 0; bottom: 0px; height: 30px; }"
      + "#SHIMCONTROLS button { cursor: pointer; }";
  var FORMAT_REGEXP = /([^%]|%([\-+0]*)(\d+)?(\.\d+)?([%sdilfox]))/g;
  var OPEN_TEXT = "Open console";
  var CLOSE_TEXT = "Close console";
  ...
  function Console() {
    ...
    controls.id = 'SHIMCONTROLS';
    (document.body || document.documentElement).appendChild(controls);
    toggle = document.createElement('button');
    toggle.appendChild(document.createTextNode(OPEN_TEXT));
    ...
    function show(args, type, prefix) {
      ...
      function repl(str, unit, flags, width, precision, specifier) {
        ...
        switch(specifier) {
          ...
          case 'i':
          case 'l':
            return String(trunc(Number(arg)));
          case 'f':
      ...
      if(typeof args[0] === 'string') {
        line.appendChild(document.createTextNode(args.shift().replace(
            FORMAT_REGEXP, repl)));
    ...
    this.open = function() {
      ...
      toggle.appendChild(document.createTextNode(CLOSE_TEXT));
    };
    ...
    this.log = function log(messageObject) {
      show(toArray(arguments), 'LOG');
    };
    this.debug = function debug(messageObject) {
      show(toArray(arguments), 'DEBUG');
    };
    this.info = function info(messageObject) {
      show(toArray(arguments), 'INFO');
    };
    this.warn = function warn(messageObject) {
      show(toArray(arguments), 'WARN', 'Warning: ');
    };
    this.error = function error(messageObject) {
      show(toArray(arguments), 'ERROR', 'Error: ');
    };
    ...
    this.timeEnd = function timeEnd(name) {
      name = (name || '');
      if(('$' + name) in times) {
        show([name + ": " + (Number(new Date) - times['$' + name]) + "ms"],
            'INFO');
        delete times['$' + name];
      }
    };
